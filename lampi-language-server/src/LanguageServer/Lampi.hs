...
module LanguageServer.Lampi where
...
import qualified Lampi.Print as Raw
...
toLambdaPiLam
  ...
  -> Raw.ScopedTerm' ann 
  -> LambdaPi ann n
toLambdaPiLam loc scope env pat (Raw.AScopedTerm _ body) =
  case pat of
    Raw.PatternWildcard ann -> Foil.withFresh scope $ \binder ->
      let scope' = Foil.extendScope binder scope
      in Lam loc binder (PatternWildcard ann) (toLambdaPi scope' (fmap Foil.sink env) body)

    Raw.PatternVar ann (Raw.VarIdent x) -> Foil.withFresh scope $ \binder ->
      let scope' = Foil.extendScope binder scope
          env' = Map.insert x (Foil.nameOf binder) (fmap Foil.sink env)
      in Lam loc binder (PatternVar ann x) (toLambdaPi scope' env' body)
...
toLambdaPiPi
  ...
  -> Raw.ScopedTerm' ann 
  -> LambdaPi ann n
toLambdaPiPi loc scope env pat a (Raw.AScopedTerm _ b) =
  case pat of
    Raw.PatternWildcard ann -> Foil.withFresh scope $ \binder ->
      let scope' = Foil.extendScope binder scope
       in Pi loc binder (PatternWildcard ann) (toLambdaPi scope env a) (toLambdaPi scope' (fmap Foil.sink env) b)

    Raw.PatternVar ann (Raw.VarIdent x) -> Foil.withFresh scope $ \binder ->
      let scope' = Foil.extendScope binder scope
          env' = Map.insert x (Foil.nameOf binder) (fmap Foil.sink env)
       in Pi loc binder (PatternVar ann x) (toLambdaPi scope env a) (toLambdaPi scope' env' b)
...
toLambdaPi scope env = \case
    ...
    Raw.Lam loc pat body -> toLambdaPiLam loc scope env pat body
    Raw.Pi loc pat a b -> toLambdaPiPi loc scope env pat a b
